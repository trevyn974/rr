...
class Scraper:
	def __init__(self):
		self.agencies = {}
		self.incident_types = {}
		with open("incident_types.json") as f:
		...
		try:
			...
			print(f"Error loading agencies from API: {e}")
			# Fallback to known agencies
			self.agencies = {
				"4600": {
					"agencyid": "4600",
	...
	def _agency_raw_data(self, a_id):
		try:
			...
			print(f"Error decrypting data for agency {a_id}: {e}")
			return {}
...
if __name__ == "__main__":
	s = Scraper()
	print("Available agencies:", s.list_agencies())
	
	# Test with Rogers Fire Department
	rogers_agencies = s.find_agencies_by_location(city="Rogers", state="AR")
	if rogers_agencies:
		agency = rogers_agencies[0]
		print(f"Testing with: {agency['agencyname']}")
		incidents = s.get_incidents(agency['agencyid'])
		...
		print("No Rogers agencies found")
