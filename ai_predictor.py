...
class SimpleIncidentMonitor:
    ...
    def _save_hotspot_history(self) -> None:
        ...
        try:
            with open(HOTSPOT_HISTORY_FILE, "w", encoding="utf-8") as f:
                json.dump({"incidents": self.hotspot_history}, f, indent=0)
        except Exception as e:
            print(f"[HOTSPOT] Could not save history: {e}")

    def _append_hotspot(self, incident: Incident) -> None:
        """Record incident location for hotspot map (if lat/lng available)."""
    ...
    def _compute_cluster_weights(self, clusters: List[dict]) -> None:
        """In-place: add weighted_score, trend (last_24h, previous_24h), and quality to each cluster."""
    ...
    def _under_served_clusters(self, clusters: List[dict]) -> List[dict]:
        """Clusters whose centroid is farther than STATION_UNDERSERVED_RADIUS_MI from every station."""
        radius_km = STATION_UNDERSERVED_RADIUS_MI * 1.60934
        out = []
        for c in clusters:
            min_d = min(_haversine_km(s[0], s[1], c["lat"], c["lng"]) for s in STATION_LOCATIONS)
            if min_d > radius_km:
                c_copy = {k: v for k, v in c.items() if k != "incidents"}
                c_copy["min_station_km"] = round(min_d, 2)
                c_copy["min_station_mi"] = round(min_d / 1.60934, 2)
                out.append(c_copy)
        return out[:20]
...
<head>
    ...
    <title>Call Density Hotspot Map</title>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css">
    <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
    <script src="https://unpkg.com/leaflet.heat@0.2.0/dist/leaflet-heat.js"></script>
    <style>
        * {{ box-sizing: border-box; }}
        body {{ margin: 0; font-family: 'Segoe UI', system-ui, sans-serif; background: #1a1a1a; color: #e5e5e5; }}
        ...
        .panel h1 {{ margin: 0 0 4px 0; font-size: 16px; font-weight: 700; color: #fff; letter-spacing: -0.02em; }}
...
<body>
    ...
    <div class="panel">
        ...
        <label class="toggle"><input type="checkbox" id="autoRefresh"> Auto-refresh every 30s</label>
        ...
        <div class="map-buttons">
            <button type="button" class="btn" id="fitAll">Fit all</button>
            <button type="button" class="btn" id="fitMyRadius">Fit my radius</button>
        </div>
        ...
        <p class="updated" id="liveNote" style="font-size:10px;color:#525252">Auto-refresh: fetches incident_hotspot_data.json every 30s when served from same folder</p>
    </div>
    <script>
        ...
        let map, heatLayer, markersLayer, stationLayer, chaserLayer, stagingLayer, coverageZonesLayer;
        ...
        function getFilteredClusters() {{
            ...
            return allClusters.map(c => {{
                ...
                const weighted = c.weighted_score != null ? c.weighted_score : incidents.length;
                return {{ lat, lng, count: incidents.length, incidents, weighted_score: weighted, last_24h: c.last_24h, previous_24h: c.previous_24h, quality: c.quality }};
            }}).filter(Boolean);
        }}
        ...
        function applyFilters() {{
            ...
            heatLayer = null;
            if (areaHeat.length > 0) {{
                heatLayer = L.heatLayer(areaHeat, {{
                    radius: 40, blur: 30, maxZoom: 16, minOpacity: 0.4, max: maxIntensity, gradient: grad
                }}).addTo(map);
            }}
            ...
            if (underServedClusters && underServedClusters.length && underServedEl && underServedPanel) {{
                ...
                underServedEl.innerHTML = underServedClusters.slice(0, 8).map(u => `<div class="busiest-addr-item" data-lat="${{u.lat}}" data-lng="${{u.lng}}"><span class="addr-text">${{u.min_station_mi}} mi from stations · ${{u.count}} call${{u.count !== 1 ? 's' : ''}}</span><span class="addr-count">${{u.count}}</span></div>`).join('');
            ...
            if (stagingSuggestion && stagingPanel && stagingContent) {{
                ...
                stagingContent.innerHTML = '<div class="staging-point" data-lat="' + stagingSuggestion.lat + '" data-lng="' + stagingSuggestion.lng + '">' +
                    '<div class="staging-desc">Center of top 3 likely-next areas</div>' +
            ...
            if (document.getElementById('showMarkers').checked && filtered.length > 0) {{
                ...
                filtered.forEach((c, idx) => {{
                    const m = L.circleMarker([c.lat, c.lng], {{
                        radius: Math.min(10, 5 + c.count),
                        fillColor: c.count > 1 ? '#ef4444' : '#f97316',
            ...
            const nearest5 = chaser && chaser.lat != null ? [...filtered].map(c => ({{ ...c, dist: chaserDist(c) }})).sort((a, b) => a.dist - b.dist).slice(0, 5) : [];
            ...
            nearestEl.innerHTML = nearest5.length ? nearest5.map((c, i) => {{
                ...
                const tier = densityTier(c.count);
                return `<div class="nearest-item" data-lat="${{c.lat}}" data-lng="${{c.lng}}"><span class="nearest-addr" title="${{addr}}">${{addr}}</span><span class="nearest-dist">${{c.dist.toFixed(1)}} mi</span></div>`;
            }}).join('') : '<p class="stat">Set your location or no data</p>';
            ...
            listEl.innerHTML = top5.map((c, i) => {{
                ...
                const tier = densityTier(c.count);
                return `<div class="top-n-item" data-lat="${{c.lat}}" data-lng="${{c.lng}}"><span class="count">#${{i+1}} ${{c.count}} call${{c.count !== 1 ? 's' : ''}}</span><span class="density-badge ${{tier.tier}}">${{tier.label}}</span><span class="addr">${{addr}}</span></div>`;
        ...
        function popupContent(cluster, clusterIndex) {{
            ...
            chaserLine += `<div class="popup-chaser"><a href="${{mapsUrl}}" target="_blank" rel="noopener" style="color:#3b82f6">Open in Google Maps</a></div>`;
            const tier = densityTier(cluster.count);
            ...
            const tierLine = '<div class="popup-count">Activity: <span class="density-badge ' + areaTier.tier + '">' + areaTier.label + '</span> (' + countText + ')' + scoreLine + qualityBadge + '</div>';
        ...
        function fetchLiveHotspotData() {{
            fetch(HOTSPOT_DATA_URL).then(r => r.ok ? r.json() : null).then(data => {{
                if (data && data.clusters) {{
                    ...
                    if (data.meta) hotspotMeta.generated_at = data.meta.generated_at; hotspotMeta.baseline_calls_this_hour_dow = data.meta.baseline_calls_this_hour_dow;
                    applyFilters();
        ...
        document.getElementById('autoRefresh').addEventListener('change', e => {{
            ...
            refreshTimer = null;
            if (e.target.checked) {{
                refreshTimer = setInterval(fetchLiveHotspotData, 30000);
        ...
        document.getElementById('copyMapLink').addEventListener('click', function() {{
            ...
            navigator.clipboard.writeText(url).then(() => alert('Map link copied to clipboard')).catch(() => alert('Copy failed'));
        }});
...
def main():
    ...
    if "--help" in argv or "-h" in argv:
        ...
        print("  (no option)     Run monitor; generate hotspot map on startup and on each new incident")
        print("  --generate-map  Write incident_hotspot_map.html from current history and exit")
        print("  --test-call     Send a test Pushover alert, add one hotspot point, regenerate map, and exit")
        return
